...
import { useUpdateSymbolMutation } from "@/redux/api/spreadApi";
...
export default function EditSymbolModal({
  symbol,
  children,
}: EditSymbolModalProps) {
  const t = useTranslations("ManageSpread"); // translation namespace
  ...
  const [updateSymbol, { isLoading: updating }] = useUpdateSymbolMutation();

  // Initialize selected countries when dialog opens or symbol changes
  useEffect(() => {
    if (open && symbol?.countries) {
      const initialSelected = countries.filter((country) =>
        symbol.countries.includes(country.country),
      );
      setSelectedSymbol(initialSelected);
    }
  }, [open, symbol]);
  ...
  const handleSubmit = async () => {
    ...
    const hasChanges =
      ...
      JSON.stringify([...symbol.countries].sort());
    ...
    try {
      ...
      toast.success(t("symbol_updated_successfully"));
      setOpen(false);
      setSearchValue("");
    } catch (err: any) {
      const msg = err?.data?.message ?? t("failed_to_update_symbol");
      toast.error(msg);
    }
  };
  ...
  const resetToOriginal = () => {
    const originalSelected = countries.filter((country) =>
      symbol.countries.includes(country.country),
    );
    setSelectedSymbol(originalSelected);
    toast.info(t("reset_to_original_selection"));
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>{t("edit_symbol")}</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex gap-2 pt-4 border-t">
          ...
          <Button
            ...
            disabled={updating || selectedSymbol.length === 0}
            className="flex-1"
          >
            {updating
              ? t("updating")
              : t("update_symbol", { count: selectedSymbol.length })}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
