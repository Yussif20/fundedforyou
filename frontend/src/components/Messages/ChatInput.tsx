...
export default function ChatInput() {
  ...
  const [addAssets, { isLoading: addAssetIsLoading }] =
    useUploadMultipleAssetsMutation();
  const [deleteAssets, { isLoading: deleteAssetIsLoading }] =
    useDeleteAssetMutation();
  ...
  return (
    <div className="p-4 border-t border-gray-200 bg-foreground">
      {/* File Preview Area */}
      {(fileUrls.length > 0 || totalLoading > 0) && (
        <div className="">
          ...
          <div className="flex flex-wrap gap-2 p-3 bg-gray-50 rounded-lg max-h-32 overflow-y-auto">
            {fileUrls.map((filePreview, idx) => (
              <div key={idx} className="relative group">
                <div className="w-16 h-16 rounded-lg overflow-hidden border-2 border-gray-200">
                  <Image
                    src={
                      `${AppConfig.backendUrl}${filePreview}` ||
                      "/placeholder.svg"
                    }
                    alt="Preview"
                    width={64}
                ...
                <button
                  ...
                  className={`absolute -top-2 -right-2 w-6 h-6 ${
                    isProcessingFiles || isSending
                      ? "bg-gray-400 cursor-not-allowed"
                      : "bg-red-500 hover:bg-red-600"
                  } text-foreground rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity`}
                >
                  <X className="w-3 h-3" />
                </button>
              </div>
            ...
            {totalLoading
              ? Array.from({ length: totalLoading }).map((_, idx) => (
      ...
      <form onSubmit={handleSendMessage} className="flex items-end space-x-3">
        ...
        <button
          ...
          } text-foreground rounded-sm flex items-center justify-center transition-colors flex-shrink-0`}
        >
          {isSending ? (
            <Loader2 className="w-4 h-4 animate-spin" />
          ) : (
