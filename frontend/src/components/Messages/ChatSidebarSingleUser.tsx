...
export default function ChatSidebarSingleUser({
  conversationUser,
}: {
  ...
  return (
    <div
      ...
      className={`flex pr-10 items-center relative p-4 hover:bg-secondary/5 cursor-pointer transition-colors ${
        isSelected ? "bg-secondary/10 border-r-3 border-secondary " : ""
      }`}
    >
      <div className="relative">
        <div className="w-10 h-10 rounded-full flex items-center justify-center overflow-hidden relative">
          ...
          <div
            className={`w-full h-full bg-gray-800 rounded-full flex items-center justify-center text-foreground font-medium absolute inset-0 ${
              conversationUser?.profile ? "hidden" : "flex"
            }`}
          >
      ...
      <div className="ml-3 flex-1 min-w-0">
        <div className="flex items-center justify-between">
          ...
          <p className="text-xs">
            {new Date(conversationUser?.lastMessageAt).toLocaleDateString()}
          </p>
        </div>
